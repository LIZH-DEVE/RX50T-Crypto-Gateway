        ...
        if (!rst_n) begin
            ...
            case (state_q)
                ST_RX_GATHER: begin
                    ...
                    if (acl_valid) begin
                        next_shift = {gather_shift_q[119:0], acl_data};
                        next_count = gather_count_q + 5'd1;

                        if (acl_last && (next_count < BLOCK_BYTES)) begin
                            tx_shift_q     <= next_shift << ((BLOCK_BYTES - next_count) * 8);
                            tx_count_q     <= next_count;
                            gather_shift_q <= 128'd0;
                            gather_count_q <= 5'd0;
                            state_q        <= ST_TX_SCATTER;
                        end else if (next_count == BLOCK_BYTES) begin
                            active_algo_q  <= i_algo_sel;
                            crypto_block_q <= next_shift;
                            gather_shift_q <= 128'd0;
                            gather_count_q <= 5'd0;
                            state_q        <= ST_ENCRYPT;
                        end else begin
                ...
                ST_ENCRYPT: begin
                    ...
                    if (active_algo_q == ALG_SM4) begin
                        ...
                        if (sm4_done) begin
                            tx_shift_q        <= sm4_result;
                            tx_count_q        <= BLOCK_BYTES;
                            sm4_start_seen_q  <= 1'b0;
                            sm4_valid_burst_q <= 3'd0;
                            state_q           <= ST_TX_SCATTER;
                        end
                ...
                ST_TX_SCATTER: begin
                    if (!bridge_valid && (tx_count_q != 5'd0)) begin
                        bridge_valid <= 1'b1;
                        bridge_data  <= tx_shift_q[127:120];
                        bridge_last  <= (tx_count_q == 5'd1);
