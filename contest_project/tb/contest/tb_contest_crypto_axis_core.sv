...
module tb_contest_crypto_axis_core;
    ...
    localparam logic [127:0] ACL_DEFAULT_SIG0 = 128'h58595A575052545558595A5750525455;
    localparam logic [127:0] ACL_BLOCK_SIG   = 128'h5152535455565758595A303132333435;
    ...
    initial begin
        ...
        if (!pmu_crypto_active && (rule_keys_flat === 1024'dx)) begin
        ...
        // Default slot 0 exact 16B signature should block before encryption.
        fork
            begin
                axis_send_frame({ACL_DEFAULT_SIG0, 896'd0}, 16, 1'b0);
            end
            begin
                axis_expect_no_output(60);
            end
        join
        ...
        // Rewrite slot 0 to a new 16B signature and reset its counter.
        acl_write_slot(3'd0, ACL_BLOCK_SIG);
        if (rule_keys_flat[127:0] !== ACL_BLOCK_SIG) begin
            $fatal(1, "rule key slot 0 did not update to ACL v2 signature");
        end
        if (rule_counts_flat[31:0] !== 32'd0) begin
            $fatal(1, "rule count slot 0 was not reset on rewrite");
        end

        // Shorter-than-16B frame must not trigger ACL v2 and must flush raw.
        fork
            begin
                axis_send_frame({ACL_BLOCK_SIG[127:64], 960'd0}, 8, 1'b0);
            end
            begin
                axis_expect_frame({ACL_BLOCK_SIG[127:64], 960'd0}, 8);
            end
        join

        wait_cycles(20);

        // Exact 16B match on rewritten slot 0 must block and increment 32-bit hit counter.
        fork
            begin
                axis_send_frame({ACL_BLOCK_SIG, 896'd0}, 16, 1'b0);
            end
            begin
                axis_expect_no_output(60);
            end
        ...
        if (rule_counts_flat[31:0] !== 32'd1) begin
            $fatal(1, "rule count slot 0 did not increment after ACL v2 block");
        end

        wait_cycles(20);
        $display("contest_crypto_axis_core Phase B test passed.");
        $finish;
    end
